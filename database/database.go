...
const (
	// Default database connection for local/docker-compose development (overridable via DATABASE_URL).
	defaultAppDSN      = "postgres://user:password@db:5432/gogo?sslmode=disable"
...
func Connect() error {
	...
	candidates := []string{}
	if connStr != "" {
		// When an explicit DSN is provided, try only that DSN to avoid silently falling back.
		candidates = append(candidates, connStr)
	} else {
		candidates = append(candidates, defaultAppDSN)
		// Fallback to common docker service name and localhost for convenience.
		candidates = append(candidates, "postgres://user:password@db:5432/gogo?sslmode=disable")
	...
	for idx, dsn := range candidates {
		for attempt := 0; attempt < 2; attempt++ {
			...
			// If no explicit DATABASE_URL and this is the first candidate that points to a local socket,
			// try to provision local Postgres once. Skip for remote defaults.
			if connStr == "" && idx == 0 && attempt == 0 && strings.Contains(dsn, "host=/tmp") {
